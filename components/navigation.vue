<template>
  <nav class="relative flex self-start rounded-full bg-slate-100 p-[1px] dark:bg-slate-800">
    <div
      aria-hidden="true"
      :class="[
        'pointer-events-none absolute inset-y-[1px] left-[1px] w-[calc(33.333%-1px)] rounded-full border border-slate-200 bg-white shadow-xs transition-transform duration-250 ease-out',
        activeIndex === 1 ? 'translate-x-full' : activeIndex === 2 ? 'translate-x-[200%]' : 'translate-x-0',
      ]" />
    <NuxtLink
      :class="[
...
const activeIndex = computed(() => {
  ...
  if (route.path.startsWith("/life")) return 2;
  return 0;
});
</script>
