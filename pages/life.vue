<template>
  <main
    ...
    <section
      ref="sectionRef"
      class="col-span-full grid gap-3 sm:grid-cols-8 sm:gap-x-8 md:grid-cols-10 lg:grid-cols-12 xl:grid-cols-16">
      <div
        v-for="(photo, index) in lifePhotos"
        :key="photo.src"
        data-flip-item
        :data-life-photo="photo.src"
        :class="layout === 'grid' ? photo.gridClass : lifeListGridClass">
        <LifeFigure
          :src="photo.src"
          :title="photo.title"
          :date="photo.date"
  ...
  <Teleport to="body">
    <template v-if="isOpen">
      <div
        class="fixed inset-0 z-[420] cursor-zoom-out bg-white transition-opacity duration-[280ms]"
    ...
    <LifeLayoutToggle @change="onLayoutChange" />
  </Teleport>
</template>

<script setup lang="ts">
import { lifeListGridClass, lifePhotos } from "~/data/life";
